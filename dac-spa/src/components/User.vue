...
export default {
  ...
  methods: {
    ...
    async loadApps() {
      const accessToken = await this.$authn.getAccessToken();
      const res = await axios.get(this.$config.api + "/apps", {
        headers: { Authorization: "Bearer " + accessToken },
      });
      this.apps = res.data;
    },
    ...
    async markApps() {
      ...
      for (const grp of this.groups) {
        ...
        if (parts[0] == "APPUSERS") assigned.push(parts[2]);
      }
    ...
    async validate_email_on_keyup() {
      ...
      this.typingDelayTimer = setTimeout(async function() {
        ...
        try {
          ...
          if (
            !self.user.id ||
            self.user.id.lenght <= 0 ||
            res.data.id != self.user.id
          ) {
            self.duplicateFound =
