  ...
  <v-card class="mx-auto my-2" max-width="850">
    ...
    >
        ...
        <v-tooltip bottom>
          ...
          <v-card flat color="transparent" class="mx-auto">
            <div class="white--text caption">
              <v-icon size="12" color="grey lighten-2" class="mr-1"
                >mdi-cog</v-icon
              >
              Manage
            </div>
      ...
      <template v-slot:item.lastLogin="{ item }">{{
        $utils.offsetDate(item.lastLogin)
      }}</template>
...
export default {
  ...
  data: () => ({
    dialog: false,
    statusFilterString:
      //"profile.approvalStatus+eq+%22pending%22",
      "profile.approvalStatus+eq+%22approved%22+and+(status+eq+%22ACTIVE%22+or+status+eq+%22STAGED%22+or+status+eq+%22RECOVERY%22+or+status+eq+%22DEPROVISIONED%22+or+status+eq+%22SUSPENDED%22+or+status+eq+%22PROVISIONED%22+or+status+eq+%22LOCKED_OUT%22+or+status+eq+%22PASSWORD_EXPIRED%22)",
    headers: [
      {
        text: "First Name",
        value: "firstName",
  ...
  methods: {
    ...
    async loadUsers() {
      ...
      params += `&sortBy=${
        this.profileFields.includes(sortBy[0])
          ? "profile." + sortBy[0]
          : sortBy[0]
      }`;
