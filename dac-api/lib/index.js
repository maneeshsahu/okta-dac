...
async function getGroup(type, name) {
  try {
    const res = await axios.get(
      orgUrl + "/api/v1/groups?q=" + type + "_" + name + "&limit=2",
      headers
    );
    if (
      res.status == 200 &&
      res.data.length == 1 &&
      res.data[0].profile.name == type + "_" + name
    ) {
      let body = res.data[0];
      delete body._links;
      return {
...
async function getPendingUsersGroup(name) {
  return getGroup("PENDING", name);
}

async function addKey(x5c) {
  try {
    const keyData = await axios.post(
      orgUrl + "/api/v1/idps/credentials/keys",
      {
        x5c: x5c,
      },
      headers
